//#include "cluster.cpp" // НЕ ИНКЛУДВАЙ ХЕДЪРИ !!!!!!!!!!!
#include "cluster.h"
...
// много добра практика - вкарваш само тези имена, които ти трябват, вместо цялото пространство с имена
using std::ifstream;
using std::ofstream;
using std::find;
using std::string;
...
// приемай clusters под константна референция !!!!!!! така се създават (и после унищожават)
// копия на всичките ти точки ВСЕКИ ПЪТ когато искаш да намериш на НЯКОЯ точка най-близкия център(!)
size_t findClosestCluster(vector<Cluster> clusters, const Point& p)
{
	//get the max value of double type

    // правилно си се досетил за _по-добрия_ начин за търсене на минимум/максимум
...
Point findCenter(const vector<Point>& points)
{
    ...
    // тъй като всеки път смяташ сумата от разстоянията от една точка до всички други,
    // то тази информация е СИЛНО ПРЕПОРЪЧИТЕЛНО да се изчисли предварително, да се запази
    // в един вектор и да не се преизчислява на всяка итерация (!) значително ще подобри времето
    // и от теоретична, и от практическа гледна точка!
...
// и тук, и във findCenter много уместно си използвал референции
void clusterize(vector<Cluster>& clusters)
{
	...
	for(size_t i = 0; i < clusters.size(); ++i)
	{
        ...
        // but why make a copy of all the points? -A
...
void writeToFile(const string& filename, const vector<Cluster>& clusters)
{
    ...
    auto& file = std::cout; // това го добавям за дебъгване и леснота
	for(size_t i = 0; i < clusters.size(); ++i)
	{
		file << i + 1<<": " << endl;
		...
		{
			file << (*it).first << ' ' << (*it).second << endl; // може it->first, it->second
		}
		file << endl;
	}
}
...
{	
	...
	if(readFile(points, "test.txt"))
	{
        ...
        // това цялото надолу можеше да бъде процедура chooseClusters или нещо подобно !
		...
		for(auto it = points.begin(); it != points.end(); ++it)
		{
            ...
            // ето тук щеше да се избегне копиране, ако getSet връщаше референция,
            // а ти беше написал const vector<Point>& curr = ...
